...
# .@FileName:convex_hull_utils
# .@Date....:2025-05-23 : 12 : 09
# .@Aurhor..:冥羽
# .@Contact.:1942598111@qq.com
'''
launch:
        import convex_hull_utils as Qs_FileName
        reload(FileName)
        FileName.main()
'''
import hou
...
def create_convex_hull(geo, points, normalize_parm = True, flip_normal_parm = True, simplify_parm = False, level_detail=0):
    '''
    Creates a convex hull based on the geo and points
    Args:
        geo = python node.geometry()
        points = the geometry points to be used
        normalize_parm, flip_normal_parm, simplify_parm and level_detail = extra parms to adjust normals and details.
    '''
    ...
    try:
        ...
        if simplify_parm and level_detail > 0:
            ...
            # Round position to grid and remove duplicate points
        ...
        for face in hull.simplices:
            ...
            for id in face:
                vertex_id = np.where(hull.vertices == id)[0][0]
            ...
            if len(face_points) >= 3 and normalize_parm:
                ...
                # Determin if we need to flip the face
        ...
        hou.ui.displayMessage("Scipy or numpy modules not found")
