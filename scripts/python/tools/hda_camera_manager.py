...
# .@FileName:hda_camera_manager
# .@Date....:2025-05-22 : 14 : 19
# .@Aurhor..:冥羽
# .@Contact.:1942598111@qq.com
'''
launch:
        import hda_camera_manager as Qs_FileName
        reload(FileName)
        FileName.main()
'''
import hou
...
class MultiCameraManager:
    ...
    def __init__(self):
        ...
        self.node = hou.pwd()
        self.CAMERA_PARMS = ["tx", "ty", "tz", "rx", "ry", "rz", "resx", "resy", "aspect",
    ...
    def scan_scene_cameras(self):
        ...
        try:
            ...
            hou.ui.displayMessage(
                f"Error scanning the scene for cameras {e}",
    ...
    def set_active_camera(self):
        '''Set specified camera as a viewport'''
    ...
    def _get_camera_frame_range(self, camera):
        '''
        Gets the frame range of a specific camera and sets the playback range
        Args:
    ...
    def merge_cameras(self):
        ...
        Camera animations are arranged sequentially with each camera's animation starting after the previous camera's last keyframe
        '''
        ...
        for cam_name, cam_data in self.sorted_cameras.items():
        ...
        if current_frame > 1001:
            hou.playbar.setPlaybackRange(1001, current_frame)
            set_global_frame_range = f"test `1001/$FPS` `{current_frame}/$FPS"
            hou.hscript(set_global_frame_range)
            hou.setFrame(1001)
    ...
    def _sorted_cameras(self):
        ...
        Returns:
            dict - dictionary containing all the cameras sorted
        '''
    ...
    def select_rendering_node(self):
        '''
        Select a Karma node and set up the rendering parameters based on a selected camera
        '''
        ...
        if not cameras_to_render:
            hou.ui.displayMessage("No valid cameras found for rendering", severity=hou.severityType.Error)
            return

        # Process each camera
        for cam_name, camera in cameras_to_render.items():
            base_output = "$HIP/render/$HIPNAME.$OS.SF4.exr"
            camera_output = base_output.replace(".$F4.exr", f".{cam_name}.SF4.exr")
