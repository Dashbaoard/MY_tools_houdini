...
# .@FileName:usd_geo_nor
# .@Date....:2025-06-12 : 08 : 32
# .@Aurhor..:冥羽
# .@Contact.:1942598111@qq.com
'''
launch:
        import usd_geo_nor as Qs_FileName
        reload(FileName)
        FileName.main()
'''
# _*_ coding: utf-8 _*_
# USD Export with Extent + Transform Names + UV Indices + Normals (Direct Normals)
...
def get_mesh_normals(maya_mesh):
    """Get vertex normals using OpenMaya API"""
    selection = om.MSelectionList()
    selection.add(maya_mesh)
    ...
    meshFn.getVertexNormals(False, normals, om.MSpace.kWorld)

    # Convert to Gf.Vec3f array
    normal_list = []
    for i in range(normals.length()):
        normal = normals[i]
        normal_list.append(Gf.Vec3f(normal.x, normal.y, normal.z))

    return Vt.Vec3fArray(normal_list)
...
def export_mesh_with_extent(stage, transform_path, shape_path, usd_path):
    """Export mesh with extent calculation and direct normals"""
    ...
    # Export Normals (direct as normals attribute)
    normals = get_mesh_normals(shape_path)
    if normals:
...
if __name__ == "__main__":
    export_path = r"E:/usd/export/v002/mod26.usda"
