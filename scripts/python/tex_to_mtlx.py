...
class TxToMtlx(QtWidgets.QMainWindow):
    ...
    def init_constants(self):
        ...
        # For materia library information
        self.node_path = '/obj/matnet1'
        self.node_lib = hou.node(self.node_path) # <..............Change this to None at the end
        self.folder_path = None

        # Texture related contents
        self.TEXTURE_EXT = ['.jpg', 'jpeg', '.png', '.bmp', '.tif', '.tiff', 'exr', 'targa']
        self.TEXTURE_TYPES = ['diffuse', 'diff', 'albedo', 'alb', 'base', 'col', 'color', 'basecolor',
                'metallic', 'metalness', 'metal', 'mlt', 'met',
    ...
    def _setup_material_section(self):
        ...
        self.bt_open_folder.setEnabled(True) # <..............Change this to False at the end.
    ...
    def help_menu(self):
        ...
        text_to_display = """
                        ...
                        \nBump: bump, bmp,
                        \nDisplacement: displacement, displace, disp, dsp, heightmap, height,
                        \nFilter: user, mask,
    ...
    def open_folder(self):
        ...
        if self.folder_path:
            if self.folder_with_textures(self.folder_path):
    ...
    def folder_with_textures(self, folder):
        """Checks to see it folder contains any valid textures"""
    ...
    def select_all_mtl(self):
        ...
        for rou in range(self.model.rowCount()):
            index = self.model.index(rou, 0)
    ...
    def create_materials(self):
        """Passes the info needed to the MtlxMateriaClass"""
        ...
        for index in selected_rows:
            ...
            # create material.placeholder_method()
...
class MtlxMaterial:
    ...
    def init_constants(self):
        ...
        'texturesMetal': ['metallic', 'metalness', 'metal', 'mll', 'met'],
        ...
        'texturesEmission': ['emission', 'emissive', 'emit', 'emm'],
        'texturesAlpha': ['alpha', 'opacity', 'opac'],
        'texturesAO': ['ao', 'ambient occlusion', 'occlusion'],
    ...
    def _convert_to_tx(self, texture_paths):
        ...
        def convert_single_texture(texture_path):
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.error(f"Thread {thread_id}: Failed to convert {os.path.basename(texture_path)} in {result}")
                    return False
            except Exception as e:
                logger.error(f"Thread {thread_id}: Error converting {os.path.basename(texture_path)}: {str(e)}")
                return False

        texture_paths = [os.path.join(self.folder_path, tex) for tex in texture_paths]
        ...
        logger.info(f"Conversion completed in {total_time}s, Successfully converted: {completed}, Failed: {failed}")
    ...
    def create_materialx(self):
        ...
        try:
            ...
            hou.ui.displayMessage(f"Error creating material {str(e)}", severity = hou.severityType.Error)
            ...
            for texture_type, texture_path in material_lib_info.items():
                if texture_type not in ["UDIM", "Size"]:
                    if isinstance(texture_path, list):
                        all_texture.extend(texture_path)

            self._convert_to_tx(all_texture)
    ...
    def _create_main_nodes(self, subnet_context):
        ...
        # Create main nodes.
        mltx_standard_surf = subnet_context.createNode("mtlxstandard_surface", self.material_to_create + 'mtlxstandard_surface')
        mltx_displacement = subnet_context.createNode("mtlxdisplacement", self.material_to_create + 'mtlxDisp')
        ...
        # Connect outputs.
        surface_out.setInput(0, mltx_standard_surf)
        displacement_out.setInput(0, mltx_displacement)

        return mltx_standard_surf, mltx_displacement

    def _create_output_nodes(self, context, output_type):
        """
        Create an output connect node.
        Args:
    ...
    def _setup_place2d(self, subnet_context, material_lib_info):
        '''
        Setup the placed2d nodes for non-UDIM textures.
        Args:
    ...
    def _process_textures(self, subnet_context, mtlx_standard_surf, mtlx_displacement, material_lib_info, place2d):
        ...
        Args:
            ...
            placed2d - if no UDIM we used this system to control the texture
        '''
    ...
    def _create_texture_node(self, subnet_context, texture_info, material_lib_info):
        ...
        # Check the node type based on the UDIM setting
    ...
    def _configure_texture_node(self, node, texture_type):
        ...
        if texture_type in ['texturesColor', 'texturesSSSS']:
            signature = "color3"
            colorspace = "srgb_tx"
    ...
    def _setup_glossiness_texture(self, texture_node, mtlx_standard_surf, input_index):
        ...
        range_node = texture_node.parent().createNode("mtlxrange", texture_node.name() + "_ADJ")
        range_node.setInput(0, texture_node)
        range_node.param("outlow").set(1)
        range_node.param("outhigh").set(0)
        mtlx_standard_surf.setInput(input_index, range_node)
    ...
    def _setup_sss_texture(self, texture_node, mtlx_standard_surf, input_index):
        ...
        range_node.parm("signature").set("color3")
        mtlx_standard_surf.setInput(input_index, range_node)
        mtlx_standard_surf.param("subsurface").set(1)
    ...
    def _setup_bump_normal(self, subnet_context, mtlx_standard_surf, material_lib_info, placed2d):
        ...
        def _create_bump():
            ...
            if placed2d and not material_lib_info.get("UDIM", True):
                bump_image.setInput(2, placed2d)
        ...
        def _create_normal():
            ...
            if placed2d and not material_lib_info.get("UDIM", True):
                normal_image.setInput(2, placed2d)
    ...
    def _find_bump_normal_textures(self, material_lib_info):
        ...
        texture_type_sorted = {
            'texturesBump': ['bump', 'bmp', 'height'],
            'texturesNormal': ['normal', 'nor', 'nrm', 'nml', 'norm']
        }
